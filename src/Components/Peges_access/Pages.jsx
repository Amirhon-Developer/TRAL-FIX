import React from "react";
import { NavLink } from "react-router-dom";
import "../Peges_access/Pages.scss";

const Pages = () => {
  return (
    <div className="container">
        <div className="jj">
          <h1 className="jj__h1" >Menu</h1>
      <div className="pages">
        <NavLink to="/airdopes" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">
            True Wireless <br /> Earbuds
          </h1>
          <img data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/Airdopes_06c1d467-6060-4e4d-9116-edf9ce9155a7_small.png?v=1674041615"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/nescbands" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">Neckbands</h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/Neckbands_06214c1a-5e30-48ea-ac14-4a6bff679f48_small.png?v=1674126581"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/smartwatch" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">
            Smart <br /> Watches
          </h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/Smartwatch_96457e1e-dfc6-4818-b6f1-a9a7130d018c_small.png?v=1674127383"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/headphones" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">
            Wireless <br /> Headphones
          </h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/Rockerz_6c017fdd-66fd-4d00-a1c9-5fd3f8536ee1_small.png?v=1674041615"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/speaker" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">
            Wireless <br /> Speakers
          </h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/Stone_1321db16-d197-40bd-be00-d165868168eb_small.png?v=1674126200"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/headphones" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">
            Wired <br /> Headphones
          </h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/bassheads-Wired-Headphones_0877b292-85ab-42ed-a055-26e1c246be73_small.png?v=1674041616"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/earphones" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">
            Wired <br /> Earphones
          </h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/Basshead_small.png?v=1674041616"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/soundbars" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">Soundbars</h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/aavante-bar_small.png?v=1674041615"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/headphones" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">
            Gaming <br /> Headphones
          </h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/Immortals_dbb4a363-0799-44e3-8b67-a275b092c501_small.png?v=1674041615"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/party" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">Party Speakers</h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/sound_bar_4f111a6a-2482-41c8-87f2-db7e0ee19e69_small.webp?v=1674041615"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/trimmer" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">Trimmers</h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/misfit_3b8fc3eb-52d6-4519-824f-22873770f747_small.png?v=1674041616"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/charger" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">Chargers</h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/Charger_small.png?v=1674041615"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/powerbank" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">Power banks</h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/powerbank_small.png?v=1674126228"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/cable" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">Cables</h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/cables_cd468fe0-9b41-4eef-b686-0785c6478534_small.png?v=1674041615"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/accessories" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">
            Car <br /> Accessories
          </h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/Car-accessories_07e4e533-2b59-4597-aa3f-110552541f0f_small.png?v=1674041615"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/airdopes" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">TRebel Range</h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/Trebel_small.png?v=1674126241"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="/headphones" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">Limited Editions</h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/limited-edition_small.png?v=1674126214"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
        <NavLink to="view" className="pages__page">
          <h1 className="pages__pageh1"data-aos="zoom-in-right">View All</h1>
          <img
          data-aos="zoom-in"
            src="https://cdn.shopify.com/s/files/1/0057/8938/4802/files/logo123_171ada23-0c67-477b-84d1-21fe1bad3909_small.png?v=1674022741"
            alt=""
            className="pages__page__img"
          />
        </NavLink>
      </div>
      </div>
    </div>
  );
};

export default Pages;
