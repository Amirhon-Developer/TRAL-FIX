...
const PowerBank_Pages = () => {
  const { addItem, getItem, removeItem } = useCart();
  const [setData] = useState([]);
  const { data } = useContext(Context);

  const newarray26 = data.filter((el) => el.category === "Cable");
  return (
    <div className="container">
      <div style={{ marginTop: "170px" }} className="maped" data-aos='fade-up'>
        {newarray26.map((el) => {
          console.log(el._id);
          return (
            <Link to={`/card/${el._id}`}>
...
export default PowerBank_Pages;
